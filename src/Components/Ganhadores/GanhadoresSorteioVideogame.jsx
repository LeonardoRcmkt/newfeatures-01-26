...
export const GanhadoresSorteioVideogame = ({ open, onClick }) => {
  ...
  useEffect(() => {
    api
      .get("/winner")
      .then((data) => {
        const filterWinnersVideogame = data.filter(ganhador => ganhador.nome_premio === "Videogame"
        );
        setData(filterWinnersVideogame);
        setFilteredData(filterWinnersVideogame);
        setLoading(false);
      })
      .catch((err) => {
