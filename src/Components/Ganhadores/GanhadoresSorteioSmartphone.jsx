...
export const GanhadoresSorteioSmartphone = ({ open, onClick }) => {
  ...
  useEffect(() => {
    api
      .get("/winner")
      .then((data) => {
        const filterWinnersSmartphone = data.filter(ganhador => ganhador.nome_premio === "Smartphone"
        );
        setData(filterWinnersSmartphone);
        setFilteredData(filterWinnersSmartphone);
        setLoading(false);
      })
      .catch((err) => {
      ...
      <div className='flex flex-col pt-0 text-base-coloralt gap-4'>
        <div id='ganhadores-table' className='  ganhadores-table '>
          <table className='w-full text-center border-collapse'>
            ...
            <tbody>
              {loading ? (
                <tr className='bg-white p-4 text-secondary-700'>
                ...
                <tr className='bg-white p-4 text-secondary-700'>
                  <td
                    colSpan={4}
                    className='mb-2 text-center p-6 text-secondary-700'
                  >
                ...
                <tr className='h-[35px]'>
                  <td colSpan={4} className='text-center bg-white text-secondary-700 py-3'>
