...
export const GanhadoresSorteioCasa = ({ open, onClick }) => {
  ...
  useEffect(() => {
    api
      .get("/winner")
      .then((data) => {
        const filterWinnersCasa = data.filter(ganhador => ganhador.nome_premio === "Casa"
        );
        setData(filterWinnersCasa);
        setFilteredData(filterWinnersCasa);
        setLoading(false);
      })
      .catch((err) => {
