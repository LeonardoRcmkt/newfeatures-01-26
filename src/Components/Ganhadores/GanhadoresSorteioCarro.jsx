...
export const GanhadoresSorteioCarro = ({ open, onClick }) => {
  ...
  useEffect(() => {
    api
      .get("/winner")
      .then((data) => {
        const filterWinnersCarro = data.filter(ganhador => ganhador.nome_premio === "Carro"
        );
        setData(filterWinnersCarro);
        setFilteredData(filterWinnersCarro);
        setLoading(false);
      })
      .catch((err) => {
